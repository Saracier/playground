...
function filterWith2(data: object[], pattern: string) {
  const regExpPattern = new RegExp(pattern);

  const searchFn = (el: elementInArray) => {
    if (typeof el === 'number') el = el.toString();

    if (typeof el !== 'string') {
      return Object.values(el).some((value) => searchFn(value));
    } else {
...
function filterWith(data: object[], pattern: string) {
  const regExpPattern = new RegExp(pattern);
  let finalArray = data.filter((element) => {
