...
function getFullQuerryUrl(query) {
    return "".concat(APIURL, "?q=").concat(query);
}
function getFileDataPath(dataName) {
    return path.join(path.dirname(process.mainModule.filename), 'data', "".concat(dataName, ".json"));
}
var functionWithFetch = function (query) { return __awaiter(_this, void 0, void 0, function () {
    var querryURL, response, data, err_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                querryURL = getFullQuerryUrl(query);
                fs.readFile(getFileDataPath(query), function (err, fileContent) {
                    if (!err) {
            ...
            case 1:
                _a.trys.push([1, 3, , 4]);
                return [4 /*yield*/, fetch(querryURL)];
            case 2:
                response = _a.sent();
                data = response.json();
                fs.writeFile(getFileDataPath(query), JSON.stringify(data), function (err) {
                    if (err) {
                        console.error(err);
                    }
                });
...
var functionWithAxios = function (query) {
